...
def run_india_drug_prediction(
    ...
    """Run India drug repurposing prediction

    Supports checkpoint resumption: saves progress after each drug,
    re-running will skip already processed drugs.

    Args:
        drug_mapping_path: Drug mapping CSV path
        output_path: Output results path
        device: Compute device
        min_score: Minimum score threshold
        top_k_per_drug: Keep top k diseases per drug
        restart: Whether to ignore previous progress and restart
    ...
    # Clear checkpoint if restart is specified
    if restart:
        checkpoint_manager.clear()
        print("Cleared previous progress, will start from beginning")
    else:
    ...
    drug_mapping = pd.read_csv(drug_mapping_path)
    print(f"Total {len(drug_mapping)} drug records")

    # Initialize predictor
    predictor = TxGNNPredictor(device=device)
    predictor.setup()

    # Run prediction (using checkpoint manager)
    predictions = predictor.predict_batch(
        drug_mapping,
        min_score=min_score,
        top_k_per_drug=top_k_per_drug,
    ...
    print(f"\nPrediction complete! Total {len(predictions)} results")
    ...
    if len(predictions) > 0:
        ...
        # Print statistics
        print("\n" + "=" * 50)
        print("TxGNN Deep Learning Prediction Summary")
        print("=" * 50)
        print(f"Total predictions: {len(result)}")
        print(f"Unique drugs: {result['drugbank_id'].nunique()}")
        print(f"Unique indications: {result['potential_indication'].nunique()}")
        print(f"Average confidence score: {result['txgnn_score'].mean():.4f}")
        print(f"Maximum confidence score: {result['txgnn_score'].max():.4f}")
        print(f"\nTip: Use drug_mapping.csv to lookup drugbank_id to India drug mapping")
